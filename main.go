...
var (
	...
	xorFlag       = flag.Int("xor", 0, "the xor value for simple encode, only using first 8 bit.")
	maxLen        = flag.Int("max-len", 0x7fffffff, "the max length of xor from the package beginning")
	timeout       = flag.Int("timeout", 30, "session timeout in seconds")
	bufferSize    = flag.Int("buffer-size", 1600, "buffer size in bytes, the max UDP package size.")
	verboseLoging = flag.Bool("verbose", false, "verbose logging")
)
...
type Forwarder struct {
	...
	mu        sync.Mutex // Mutex to protect access to sessions map
}
